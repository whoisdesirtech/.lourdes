<?php
/**
 * Admin Settings Page & Interactive Snippet Generator (Creators API)
 */
...
class AA_Admin_Settings {
	...
	public function render_admin_page() {
		...
		<div class="wrap aa-admin-wrap">
			...
			<p class="description">
				<?php esc_html_e( 'Configure your Amazon Creators API credentials and generate custom PHP code snippets and shortcodes for your WordPress site.', 'amazon-associates-snippets' ); ?>
			</p>
			...
			<h2 class="nav-tab-wrapper">
				<a href="?page=amazon-snippets&tab=credentials" class="nav-tab <?php echo 'credentials' === $active_tab ? 'nav-tab-active' : ''; ?>">
					<?php esc_html_e( 'Creators API Credentials', 'amazon-associates-snippets' ); ?>
				</a>
	...
	private function render_credentials_tab() {
		...
		<form method="post" action="options.php">
			...
			<h3><?php esc_html_e( 'Amazon Creators API OAuth 2.0 Credentials', 'amazon-associates-snippets' ); ?></h3>
			<p class="description" style="max-width: 700px;">
				<?php esc_html_e( 'Create these under Amazon Associates Central > Tools > Creators API. Do not reuse your old PA-API Access Key / Secret Key. The plugin authenticates with OAuth 2.0 client-credentials and caches access tokens automatically.', 'amazon-associates-snippets' ); ?>
			</p>
			<table class="form-table">
				<tr>
					<th scope="row"><label for="aa_credential_id"><?php esc_html_e( 'Credential ID', 'amazon-associates-snippets' ); ?></label></th>
					<td>
						<input type="text" name="aa_credential_id" id="aa_credential_id" value="<?php echo esc_attr( $credential_id ); ?>" class="regular-text" placeholder="amzn1.application-oa2-client.xxxxxxxx" autocomplete="off" />
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="aa_credential_secret"><?php esc_html_e( 'Credential Secret', 'amazon-associates-snippets' ); ?></label></th>
					<td>
						<input type="password" name="aa_credential_secret" id="aa_credential_secret" value="<?php echo esc_attr( $credential_secret ); ?>" class="regular-text" placeholder="••••••••••••••••••••••••" autocomplete="off" />
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="aa_credential_version"><?php esc_html_e( 'Credential Version', 'amazon-associates-snippets' ); ?></label></th>
					<td>
		...
		<button type="button" id="aa-clear-cache-btn" class="button button-secondary">
			<span class="dashicons dashicons-trash" style="vertical-align: middle;"></span> <?php esc_html_e( 'Purge All Cached Amazon Items & Tokens', 'amazon-associates-snippets' ); ?>
		</button>
...
?&gt;</code>
	...
	private function render_tester_tab() {
		...
		<div class="aa-tester-card">
			<h2><span class="dashicons dashicons-testimonial"></span> <?php esc_html_e( 'Amazon Creators API Connection Diagnostics', 'amazon-associates-snippets' ); ?></h2>
			<p><?php esc_html_e( 'Test your Creators API credentials (OAuth 2.0 Bearer Token) in real-time. The access token is fetched and cached automatically.', 'amazon-associates-snippets' ); ?></p>
	...
	public function ajax_clear_plugin_cache() {
		...
		wp_send_json_success( array( 'message' => 'All Amazon item transients & Creators API token caches cleared!' ) );
	}
}
